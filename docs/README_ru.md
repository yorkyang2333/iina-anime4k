...
Мы приветствуем pull request'ы и участие сообщества в развитии проекта! Перед началом разработки обязательно изучите наше [**Руководство для разработчиков (CONTRIBUTING.md)**](../CONTRIBUTING.md), где подробно описана настройка локального окружения, правила обновления версий (`ghVersion`) и автоматизированная архитектура сборки релизов IINA.
...
* `generate-pref.py` — Генератор статической страницы настроек на 54 языках (создает `pref.html`).
